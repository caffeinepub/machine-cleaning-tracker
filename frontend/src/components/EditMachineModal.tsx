...
export function EditMachineModal({ machine, open, onOpenChange }: EditMachineModalProps) {
    ...
    useEffect(() => {
        if (open) {
            setForm({
                name: machine.name,
                lastCleaningDone: dateToInputValue(timeToDate(machine.lastCleaningDone)),
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                ...
                <DialogFooter className="gap-2">
                    <Button variant="outline" onClick={() => onOpenChange(false)} disabled={updateMachine.isPending}>
                        Cancel
                    </Button>
                    <Button onClick={handleSubmit} disabled={updateMachine.isPending} className="gap-2">
                        {updateMachine.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                        Save Changes
                    </Button>
                </DialogFooter>
            </DialogContent>
