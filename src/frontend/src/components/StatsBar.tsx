...
export function StatsBar({ machines }: StatsBarProps) {
  ...
  const ok = machines.filter(
    (m) => getMachineStatus(m.nextDue) === "ok",
  ).length;
  const dueSoon = machines.filter(
    (m) => getMachineStatus(m.nextDue) === "due-soon",
  ).length;
  const dueToday = machines.filter(
    (m) => getMachineStatus(m.nextDue) === "due-today",
  ).length;
  const overdue = machines.filter(
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
      {stats.map(({ label, value, icon: Icon, color, bg, pulse }) => (
        ...
        >
          <Icon
            className={`w-5 h-5 flex-shrink-0 ${color} ${pulse ? "animate-bounce" : ""}`}
          />
          <div>
            <p
              className={`text-xl font-condensed font-bold leading-none ${color}`}
            >
              {value}
            </p>
